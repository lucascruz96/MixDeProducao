...
public class Solver {
	...
	public ResponseEntity<?> resolver(@RequestBody Problema problema) {
        ...
        for(int i = 0; i < problema.getProdutos().get(0).getIngredientes().size(); i++) {
            ...
            GLPK.glp_set_row_bnds(lp, (i+1), GLPKConstants.GLP_DB, 0, problema.getProdutos().get(0).getIngredientes().get(i).getDisponibilidade());
            
            ind = GLPK.new_intArray(problema.getProdutos().size() + 1);
            val = GLPK.new_doubleArray(problema.getProdutos().size() + 1);
            
            for(int j = 0; j < problema.getProdutos().size(); j++) {
            	GLPK.intArray_setitem(ind, (j+1), (j+1));
            	GLPK.doubleArray_setitem(val, (j+1), problema.getProdutos().get(j).getIngredientes().get(i).getGasto());
            }
            
            GLPK.glp_set_mat_row(lp, (i+1), 2, ind, val);
    	}
